...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # 使用选项卡来组织不同的分析
            tab1, tab2, tab3 = st.tabs(["📊 基础分析", "🔥 Asin星级分析", "📈 趋势分析"])
            ...
            with col2:
                try:
                    # 创建一个包含所有评分分布饼图的HTML文件
                    all_pie_charts_html = ""
                    for i, fig in enumerate(pie_charts):
                        all_pie_charts_html += fig.to_html(full_html=False)
                        if i < len(pie_charts) - 1:
                            all_pie_charts_html += "<hr>"
                    
                    st.download_button(
                        label="📊 下载评分分布饼图",
                        data=all_pie_charts_html,
                        file_name="rating_distribution_pies.html",
                        mime="text/html",
                        help="下载HTML格式的交互式评分分布饼图"
                    )
                except:
                    st.info("评分分布饼图暂不可下载")
